...
*/
list_t *node_sta_with(list_t *node, char *prefix, char c)
{
	char *p = NULL;

	while (node)
	{
		p = sta_with(node->str, prefix);
		if (p && ((c == -1) || (*p == c)))
		return (node);
		node = node->next;
	}
...
ssize_t get_node_ndex(list_t *head, list_t *node)
{
	size_t i = 0;
